...
class CLRVectorizer(object):
    ...
    def vectorize(self, row_dict, max_seq_length):
        ...
        indices = begin_index + indices + end_index
        source_vector[:len(indices)] = indices
        source_vector[len(indices):] = self.source_vocab.mask_index
    ...
    def from_freq_dict(cls, source_freq_dict, target_freq_dict, cutoff):
        ...
        for label in list(target_freq_dict.keys()):
